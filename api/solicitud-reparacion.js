...
function emailBody(request) {
  const portalUrl = `https://www.autokeysremapspro.es/mi-solicitud.html#token=${encodeURIComponent(request.seguimiento_token)}`;
  return `<!doctype html><html><body style="margin:0;background:#08080a;font-family:Arial,sans-serif"><table role="presentation" width="100%"><tr><td align="center" style="padding:32px 14px"><table role="presentation" width="540" style="max-width:540px;width:100%;background:#111114;border:1px solid #29292f;border-radius:14px"><tr><td style="padding:30px;color:#f5f5f7"><p style="color:#ef3641;font-size:11px;font-weight:800;letter-spacing:1.5px">SOLICITUD RECIBIDA</p><h1 style="font-size:25px;margin:0 0 16px">${html(request.numero)}</h1><p style="color:#b4b4bc;line-height:1.7">Hola ${html(request.nombre)}, hemos recibido tu caso. <b style="color:#fff">No envíes todavía la unidad.</b> Primero comprobaremos la información y te indicaremos el siguiente paso.</p><div style="background:#19191d;border-radius:10px;padding:16px;margin:20px 0;color:#ddd"><p><b>Vehículo:</b> ${html(request.marca)} ${html(request.modelo)}</p><p><b>Unidad:</b> ${html(request.tipo_unidad)}</p><p><b>Solicitud:</b> ${html(request.trabajo_solicitado)}</p></div><p style="margin:22px 0"><a href="${html(portalUrl)}" style="display:inline-block;background:#e52531;color:#fff;text-decoration:none;padding:13px 19px;border-radius:8px;font-weight:800">Ver estado de mi solicitud</a></p><p style="color:#8d8d96;font-size:12px;line-height:1.6">Este enlace es privado. Conserva la referencia ${html(request.numero)} para cualquier consulta.</p></td></tr></table></td></tr></table></body></html>`;
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    return res.status(201).json({ id: request.id, numero: request.numero, seguimiento_token: request.seguimiento_token, vinculada: !!user, email_enviado: !!process.env.RESEND_API_KEY && notifications[0].status === 'fulfilled' });
  } catch (error) {
