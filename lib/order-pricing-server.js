...
async function loadCoupon(code) {
  ...
  const response = await adminRequest(
    `tienda_cupones?codigo=eq.${encodeURIComponent(couponCode)}&activo=eq.true&select=id,codigo,tipo,valor,fecha_inicio,fecha_fin,importe_minimo,usos_maximos,usos_actuales,solo_primer_pedido&limit=1`
  );
...
async function hasPriorOrders(userId) {
  const response = await adminRequest(
    `tienda_pedidos?usuario_id=eq.${encodeURIComponent(userId)}&select=id&limit=1`
  );
  const rows = await jsonOrThrow(response, 'pedidos_no_disponibles');
  return rows.length > 0;
}

async function couponDiscount(coupon, subtotal, userId) {
  if (!coupon) return 0;
  ...
  if (coupon.solo_primer_pedido) {
    if (!userId) throw new Error('cupon_solo_primer_pedido');
    if (await hasPriorOrders(userId)) throw new Error('cupon_solo_primer_pedido');
  }
...
async function quoteOrder(items, couponCode, userId) {
  ...
  const descuento = await couponDiscount(coupon, subtotal, userId);
