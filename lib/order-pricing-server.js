...
async function loadCatalogPricing() {
  const [productsRes, variantsRes, configRes] = await Promise.all([
    adminRequest('tienda_productos?activo=eq.true&select=id,name,category_id,price_from,requiere_envio,peso_envio_kg,excluido_envio_gratis,es_digital,digital_gratis'),
