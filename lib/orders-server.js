...
async function obtenerCatalogo(items) {
  ...
  const [productsRes, variantsRes] = await Promise.all([
    adminRequest(`tienda_productos?id=in.(${productFilter})&activo=eq.true&select=id,name,category_id,is_product,is_software,requiere_envio,peso_envio_kg,voluminoso,excluido_envio_gratis`),
...
async function calcularPedido({ items: rawItems, codigo_postal, pais, cupon }) {
  ...
  for (const item of items) {
    ...
    const unitWeight = physical ? (configuredWeight > 0 ? configuredWeight : Number(config.envio_peso_fallback_kg)) : 0;
    ...
    lineas.push({ item, product, variant, precio, requiere_envio: physical, peso_unitario_kg: unitWeight });
  }
